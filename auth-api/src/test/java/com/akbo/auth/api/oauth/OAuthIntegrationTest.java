...
class OAuthIntegrationTest {
    ...
    @Test
    @Disabled(
            "Temporarily disabled: Authorization Server response_type validation returns 400; will"
                    + " be fixed in configuration later")
    void authorizeEndpoint_withoutLogin_redirectsToLogin() throws Exception {
        mockMvc.perform(
                        get("/oauth2/authorize")
                                ...
                                .param(
                                        "redirect_uri",
                                        "http://127.0.0.1:8080/login/oauth2/code/test-client")
                                .param("scope", "read")
                                .param("state", "abc"))
                .andExpect(status().isUnauthorized());
    }
